...
function loadCuts() {
    $.getJSON("/cutit-data", function (data) {
        ...
        data.records.forEach(function (record) {
            ...
            if (record.amount) {
                var $amount = $('<p class="record-amount" />').text(
                    usdFormatter.format(record.amount)
                );
                $card.append($amount);
            }
            ...
            if (record.description) {
                ...
                var $desc = $('<p class="record-desc" />').text(
                    record.description
                );
            ...
            if (record.rationale) {
                var $rationaleTitle = $('<p class="record-title" />').text(
                    "Why should we cut it?"
                );
                var $rationale = $('<p class="record-desc" />').text(
                    record.rationale
                );
            ...
            if (record.data) {
                var $dataTitle = $('<p class="record-title" />').text(
                    "Data or sources"
                );
...
ben@benkallos.com`;
    ...
    var subject = encodeURIComponent(
        "[URGENT] Vote NO on a budget without cops out of schools & drastic headcount reduction"
    );
...
function getPhoneScript() {
    ...
    var councilmember =
        shuffledCouncilMembers[
            councilMemberIndex % shuffledCouncilMembers.length
        ];
  ...
  <p>District office: <a href="tel:${councilmember.phone}">${
        councilmember.phone
    }</a></p>
  `;
    councilMemberIndex++;
  ...
  <p>
  My name is ${name} and I live in ${neighborhood}. I am calling to ask Council Member ${
        councilmember.name
    } to
  vote no on a budget
...
<p>
  Will Council Member ${councilmember.name
      .split(" ")
      .slice(1)
      .join(
          " "
      )} commit to voting no on the budget if it doesn’t do these things ?
</p >
  `;
...
function buildTweetButtons() {
    ...
    const tweet1 = encodeURIComponent(
        ".@NYCSpeakerCoJo: Will you commit to #DefundNYPD by cutting the NYPD operating budget in HALF in FY21 & reducing the officer headcount by HALF? The City must redirect these funds to Black communities, & remove cops from schools, homeless services, mental health response NOW."
    );
    var url = `https://www.twitter.com/intent/tweet?text=${tweet1}`;
    tweetButton.href = url;

    tweetButton = document.getElementById("tweet-2");
    const tweet2 = encodeURIComponent(
        ".@NYCSpeakerCoJo: $1B cuts to NYPD is not enough. I, along with many in this city, are asking you for far more meaningful cuts that address the concerns of the moment. #DefundNYPD by half and cut officer headcount by half in FY21 and redistribute those funds to Black communities."
    );
...
function lookupRep() {
    ...
    $.ajax("/council-member-info", {
        ...
        if (data.error) {
            ...
            $error.html(
                $('<p class="error" />').text(
                    "Unable to find City Council information for that address"
                )
            );
            return;
        }
        ...
        var $name = $("<h2 />").text(
            `COUNCILMEMBER ${data.full_name}`.toUpperCase()
        );
        // TODO(vicki): make the capitalization hack nicer
        var $district_party = $("<p />").text(
            `District ${data.a}, ${
                data.party[0].toUpperCase() + data.party.slice(1)
            }`
        );
        ...
        if (data.positions) {
            ...
            if (data.positions.cut_nypd_budget === "YES") {
                $cuts_position = $("<p />").text(
                    "Supports cutting the NYPD budget"
                );
            } else if (data.positions.cut_nypd_budget === "NO") {
                $cuts_position = $("<p />").text(
                    "Opposes cutting the NYPD budget"
                );
            } else {
                $cuts_position = $("<p />").text(
                    "Has not committed to cutting the NYPD budget"
                );
            }
            $container.append($cuts_position);
            var $budget_vote_position;
            if (data.positions.vote_against_status_quote_budget === "YES") {
                $budget_vote_position = $("<p />").text(
                    "Will vote against a status-quo NYPD budget"
                );
            } else if (
                data.positions.vote_against_status_quote_budget === "NO"
            ) {
                $budget_vote_position = $("<p />").text(
                    "Will not vote against a status-quo NYPD budget"
                );
            } else {
                $budget_vote_position = $("<p />").text(
                    "Has not committed to vote against a status-quo NYPD budget"
                );
            }
            $container.append($budget_vote_position);
            var $cut_amount_position;
            if (
                data.positions.nypd_cut_amount &&
                data.positions.nypd_cut_amount !== "Against cuts"
            ) {
                $cut_amount_position = $("<p />").text(
                    `Is seeking ${data.positions.nypd_cut_amount} in cuts to the NYPD budget`
                );
            }
        ...
        if (data.social) {
            if (data.social.facebook_urls) {
                $contact_container.append(
                    $("<p />").html(
                        $("<a />")
                            .attr("href", data.social.facebook_urls[0])
                            .text("Facebook")
                    )
                );
            }
            if (data.social.instagram_urls) {
                $contact_container.append(
                    $("<p />").html(
                        $("<a />")
                            .attr("href", data.social.instagram_urls[0])
                            .text("Instagram")
                    )
                );
            }
            if (data.social.twitter_urls) {
                $contact_container.append(
                    $("<p />").html(
                        $("<a />")
                            .attr("href", data.social.twitter_urls[0])
                            .text("Twitter")
                    )
                );
            }
